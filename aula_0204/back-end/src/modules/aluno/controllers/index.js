...
class AlunoController{
    static async criar(requisicao,resposta){
        try {
            ...
            resposta.status(500).json({mensagem:"Erro ao criar o aluno!",
                erro: error.message})
        }
    }
    static async editar(requisicao,resposta){
        try {
            ...
            resposta.status(500).json({mensagem:"Erro ao editar o aluno!",
                erro: error.message})
        }
    }
    static async listarTodos(requisicao, resposta) {
        try {
            ...
            if(alunos.length === 0){
                return resposta.status(400).json({mensagem: "Não existem alunos para serem exibidos"})
            }
            return resposta.status(200).json(alunos);
        } catch (error) {
            resposta.status(500).json({mensagem:"Erro ao listar o aluno!",
                erro: error.message})
        }
    }
    ...
    static async listarPorMatricula(requisicao,resposta){
        try {
            ...
            resposta.status(500).json({mensagem:"Erro ao listar o aluno pela matrícula!",
                erro: error.message}) 
        }
    }
    static async excluirTodos(requisicao,resposta){
        try {
            ...
            resposta.status(200).json({mensagem: "Todos alunos excluídos com sucesso"})
        } catch (error) {
